"""
Phase 1 S1-v2: Scale Orbit Validation with High Density Sampling

Key changes from v1:
- N=21 (was N=11) → random baseline PC1 drops from 10% to 5%
- Compare aligned vs unaligned trajectories
- Use geometry-based metrics (not just PC1%)

Based on diagnostic findings:
- Git Re-Basin alignment HURTS Scale structure (1.01x compression)
- Unaligned Scale PC1=19.77%, r=0.76 vs scale parameter
- Need higher N to distinguish signal from noise
"""
...
def full_diagnosis(theta_traj, group_params, group_name, tag="", save_dir=None):
    ...
    sc = axes[0, 0].scatter(proj[:, 0], proj[:, 1], c=group_params, cmap='viridis', s=40)
    axes[0, 0].plot(proj[:, 0], proj[:, 1], 'k-', alpha=0.3, linewidth=0.8)
    plt.colorbar(sc, ax=axes[0, 0], label='Scale factor')
    axes[0, 0].set_xlabel('PC1')
    axes[0, 0].set_ylabel('PC2')
    axes[0, 0].set_title(f'{group_name} {tag}: PC1 vs PC2')
    axes[0, 0].set_aspect('equal')

    axes[0, 1].scatter(group_params, proj[:, 0], c='blue', s=40)
    z = np.polyfit(group_params, proj[:, 0], 1)
    p = np.poly1d(z)
    x_line = np.linspace(min(group_params), max(group_params), 100)
    axes[0, 1].plot(x_line, p(x_line), 'r--', alpha=0.5)
    axes[0, 1].set_xlabel('Scale factor')
    axes[0, 1].set_ylabel('PC1 projection')
    axes[0, 1].set_title(f'PC1 vs Scale (r={pc1_param_corr:.3f})')
    ...
    axes[1, 1].set_title(f'PC2 vs Scale (r={pc2_param_corr:.3f})')

    plt.tight_layout()
    fname = f'scale_N{N}_{tag}.png'
    if save_dir:
...
def run_experiment(image_path, seed=42, device="cuda", save_dir=None):
    ...
    scale_params = [round(float(x), 2) for x in np.linspace(0.5, 1.5, 21)]

    print(f"\n{'='*60}")
    print(f"Scale Orbit v2: N={len(scale_params)}, image={Path(image_path).name}")
    print(f"{'='*60}")

    models_params = []
    psnrs = []

    for i, s in enumerate(scale_params):
        coords, target = get_scaled_data(image_tensor, s, device)
        ...
        print(f"  [{i+1}/{len(scale_params)}] scale={s:.2f}: PSNR={psnr:.1f} dB")
    ...
    res_unaligned = full_diagnosis(unaligned_flat, scale_params, "Scale",
                                   tag="unaligned", save_dir=save_dir)
    res_aligned = full_diagnosis(aligned_flat, scale_params, "Scale",
    ...
    if save_dir:
        os.makedirs(save_dir, exist_ok=True)
        with open(os.path.join(save_dir, "scale_orbit_v2_results.json"), 'w') as f:
            json.dump(combined, f, indent=2)

    print(f"\n{'='*60}")
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"  N = {len(scale_params)}, Random baseline = {100/(len(scale_params)-1):.1f}%")
...
def main():
    parser = argparse.ArgumentParser(description="Phase 1 S1-v2: Scale Orbit High Density")
    parser.add_argument("--image", type=str, default="Data/Set5/HR/baby.png")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--save_dir", type=str, default="results/Phase1/Scale_Orbit_v2")
    args = parser.parse_args()

    run_experiment(args.image, seed=args.seed, device=args.device, save_dir=args.save_dir)
