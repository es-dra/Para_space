"""
Phase 1 S1: Scale Parameter Orbit Validation
...
from experiments.config import (
    SIREN_CONFIG, MAML_CONFIG, DATA_CONFIG, SCALE_CONFIG,
...
def get_scaled_image_and_coords(image_tensor, scale_factor, device):
    import cv2
...
def run_stage_a(image_tensor, task_scales, device, seed=42):
    print("\n" + "="*60)
    ...
    for i, scale in enumerate(task_scales):
        coords, target = get_scaled_image_and_coords(image_tensor, scale, device)
        ...
        print(f"  scale={scale:.1f}: PSNR={psnr:.1f} dB")
    ...
    pca = PCA(n_components=min(ANALYSIS_CONFIG["pca_components"], len(task_scales) - 1))
    pca.fit(flat_params)
...
def run_stage_b(image_tensor, task_scales, device, seed=42):
    print("\n" + "="*60)
    ...
    for step in tqdm(range(MAML_CONFIG["meta_steps"]), desc="MAML Meta-Train"):
        ...
        if step % 500 == 0 and step > 0:
            with torch.no_grad():
                psnr = -10 * np.log10(F.mse_loss(meta_model(coords), target).item() + 1e-10)
            tqdm.write(f"  Step {step}: PSNR@scale={scale:.1f} = {psnr:.1f} dB")
...
def run_experiment(image_path, seed=42, device="cuda", save_dir=None):
    ...
    task_scales = SCALE_CONFIG["param_range"]

    print(f"\n{'='*60}")
    print(f"Image: {Path(image_path).name}, Seed: {seed}")
    print(f"Transform: Scale (R+), {len(task_scales)} tasks")
    ...
    results_a, _ = run_stage_a(image_tensor, task_scales, device, seed=seed)
    results_b = run_stage_b(image_tensor, task_scales, device, seed=seed)
    ...
    if save_dir:
        ...
        save_path = Path(save_dir) / f"scale_{Path(image_path).stem}_seed{seed}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Phase 1 S1: Scale Orbit Validation")
    ...
    results_base = args.results_dir or os.path.join(OUTPUT_CONFIG["results_dir"], "Scale_Orbit")
